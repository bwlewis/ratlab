...
SEXP MexStringCellArray2SexpVector(const mxArray *pMexMat)
{
  ...
  for (i=0; i < nStrings; ++i)
  {
    ...
    {
      Rf_endEmbeddedR(0);
      printf("Element %d must be a string", i);
      UNPROTECT(1);
      return(R_NilValue);
    }
  }
...
void mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
  ...
  // to a matrix.
  RExecuteString( "ratlabReturn = matrix(as.double(ratlabReturn), nrow=nrow(ratlabReturn), ncol=ncol(ratlabReturn), dimnames=dimnames(ratlabReturn))" );
  
  // Now get the R matrix and return it as an Octave struct.
...
//  plhs[0] = mxCreateStructArray(3, pCaDims, 3, fieldNames);
  ...
  {
    mxSetCell(pColNameCells, i, mxCreateString(CHAR(STRING_ELT(rColNames, i))));
...
//      mxCreateString(CHAR(STRING_ELT(rColNames, i))));
  ...
  {
    mxSetCell(pRowNameCells, i, mxCreateString(CHAR(STRING_ELT(rRowNames, i))));
    // TODO: mxSetFieldByNumber will be more efficient.
...
//      mxCreateString(CHAR(STRING_ELT(rRowNames, i))));
  ...
  mxArray *pRetMat=mxCreateNumericArray(2, matrixDims, mxDOUBLE_CLASS, mxREAL);
